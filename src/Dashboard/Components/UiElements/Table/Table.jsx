        ...
        <tbody>
          {
            data?.length === 0 || data?.docs?.length === 0
              ? [...Array(10)].map((arr, i) => <tr key={i} className='w-full hover:bg-[#FEF9DC]'>
                <th className='py-8 border-b bg-[length:400%] bg-gradient-to-r from-gray-200 via-white to-gray-200 animate-loading' />
                {
                  tableHeadData?.map((item, index) => (
