...
const Category = ({ data }) => {
  ...
  const lgComponent = () => <div className="flex flex-col gap-2">
          ...
          <div className="ml-8 my-2">
            <ul className=" flex gap-2 flex-col list-disc">
              {category.subCategory.map((subCategory) => (
                <li className="text-[#475569] text-base font-normal cursor-pointer" key={subCategory.slug}>{subCategory.name}</li>
              ))}
            </ul>
          </div>
        )}
  ...
  const smComponent = () => <div className='lg:hidden flex items-center border rounded-full px-5 py-3'>
    ...
    >
      <p>Sort and Filter</p>
      <img src={arrow} className={`duration-300 ${open ? 'rotate-90' : '-rotate-90'}`} />
    </div>
  </div>


  return <>
    <div className='hidden lg:block'>{lgComponent()}</div>
    <div className='lg:hidden'>{smComponent()}</div>
  </>;
};
